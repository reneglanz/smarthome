...
public class CronPattern {
  ...
  private static class Parser {
    ...
    private static CronPattern.Matcher parseToken(String token, int index) throws CoreException {
      ...
      try {
        ...
        return new CronPattern.ArrayMatcher(Collections.singletonList(Integer.valueOf(CronPattern.getValue(index, token))));
  ...
  public boolean match(long time) {
    ...
    int month = cal.get(2);
    int w = cal.get(7);
    return (this.minute.match(min) && this.hour
      .match(h) && this.day
      .match(d) && this.month
      .match(month) && this.weekday
      .match(w));
  }
